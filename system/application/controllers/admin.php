	...
	function virtual_items($config = null, $name = null) {
		...
		if ($config && !$config != 'config') {
			...
			if (file_exists($source_path)) {
				...
				require($source_path);
				$data['config'] = true;
				$data['raw_config'] = file_get_contents($source_path);
				$data['raw_config'] = preg_replace("/\&/","&amp;",$data['raw_config']);
				$data['raw_config'] = preg_replace("/</","&lt;",$data['raw_config']);
				$feed = '';
				if (is_array($vi['feed'])) {
				...
				$data['config_params'] = array(
					array('paramater' => 'Title ID', 'value' => $vi['title_id']),
					array('paramater' => 'Feed Type', 'value' => $vi['feed_type']),
					array('paramater' => 'Feed', 'value' => $feed),
					array('paramater' => 'Copyright', 'value' => $vi['copyright']),
					array('paramater' => 'Creative Commons', 'value' => $vi['creative_commons']),
				);

			} else {
				$this->session->set_userdata('errormessage', 'Virtual Item with Source '.strip_tags($name).' not found.');
				redirect($this->config->item('base_url').'admin/virtual_items');					
			}
			...
			foreach ($dir as $fileinfo) {
				...
				if ($fileinfo->isDir()) {
					...
					$source = array(
						'url' => $this->config->item('base_url')."admin/virtual_items/".$dirs[3],
						'config_url' => $this->config->item('base_url')."admin/virtual_items/config/".$dirs[3],
						'name' => $dirs[3],
						'path' => $source_path,
						'item_count' => 0,
						'valid' => false,
					...
					if (file_exists($source_path)) {
						...
						// Count the items
						$query = $this->db->query("select count(*) as total from custom_virtual_items where source = ".$this->db->escape($dirs[3]));
						$row = $query->result();					
